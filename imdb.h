...
class imdb {
 ...
 private:
  void readActorInformation(char * start, vector <film>& films, int num) const;
  void readFilmInformation(char * pointer, int length, vector <string> &players) const;
  string getActorName(int offset) const;
  film getFilmInfo(int num) const;
  static const char *const kActorFileName;
  static const char *const kMovieFileName;
  static const int PLUS_FOR_MOVIE = 1900;
